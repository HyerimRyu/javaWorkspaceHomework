...
public class Main {
	...
	public static void main(String[] args) {
		...
		//위처럼 자료형끼리 묶어서 사용하는 방식은 적합하지 않을수도 있어 
		//그래서 연관있는 data끼리(이름, 국어, 영어, 평균)끼리 묶어서 관리가 더 효과적
		
		//그래서~~~class쓰는거야~!! Student Class만들자!!
		//연관있는 데이터끼리의 묶음을 설계하는 class 정의
		//새로운 클래스를 정의(설계)하려면 새로운 .java 파일에 작성 권장
		...
		Student stu2=new Student(); //2번째꺼 만들때는 변수명 다르게해서 
		stu2.name="robin";
		stu2.kor=100;
		stu2.eng=80;
		...
		//화면출력
		// System.out.println(stu);//참조변수가 가지고 있는 객체의 주소(실제 주소 아님!)
		...
		//객체의 값 변경
		int ko=10;
		stu2.kor=ko; //변수명을 쓰면 변수가 갖고 있는 값에 신경써여
		...
		System.out.println();
		//이렇게 복붙으로 객체값 출력하는 건 실수의 여지가 있어!!! 
		
		//이런식으로 출력할수 있다면?!
		//stu3.출력해(); //이런식의 기능이 있다면...
		//다시말해, 객체가 기능도 있다면.. 더 편하게 프로그래밍이 가능하겠지!!!
		
		//자바는 클래스 안에 기능도 가질 수(설계할 수) 있도록 되어 있음.
		//Student클래스 안에 기능추가 해 보기!
		stu3.show(); //기능(메소드)호출
		 
		//show()처럼 기능을 호출할 수 있으니
		//멤버변수들의 값을 출력하는 기능도 만들어 호출할 수 있어
		stu3.output();
		
		
		//이럭식이면.. 평균 계산하는거... ... 도 기능으로!!!!!!!!!!! 하면 더 편하지요
		...
		//위처럼 메소드를 잘 사용하면 코드가 훨씬 간결하고 가독성도 좋아져!
		
		//이런식이면..
		//멤버값을 하나씩 입력하는 기능도 역시 만들 수 있겠지!
		//즉, 한줄에 멤버값을 입력하는 기능!! 
		//stu4= {"lee", 100, 50}; //이것도 이상함! 이것은 배열의 초기화 문법! 
	
		//stu4.입력기능("lee", 100, 50); //이런식으로 기능이 있으면 더 간결함! 
		//즉, 메소드의 파라미터에 값을 전달하는 기능!! 
		
		//이게 된다면.. 이런식으로 ..(메소드의 파라미터에 값을 전달!) 
		//메소드를 왜 만들고 왜 써야하는지..
		
		//Student stu5=new Student();
		//stu5.입력가능("park", 100, 50); //이때 평균도 이미 계산이 되어야 함.
		//stu5.출력기능(); //입력값들 출력! 
	}
